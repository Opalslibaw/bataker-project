...
const IconBot = () => (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
    ...
    <path d="M13 19l6-6"/><path d="M16 16l4 4"/><path d="M19 21l2-2"/><path d="M3 21l9-9"/>
  </svg>
)
const IconGlobe = () => (
  <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.6" strokeLinecap="round" strokeLinejoin="round">
...
function StatCard({ label, value, color, glow, delay, icon }) {
  return (
    <motion.div
      ...
      style={{ border: `1px solid ${color}25`, background: 'rgba(0,0,0,0.55)' }}>
      ...
      <div className="relative">
        <div className="flex justify-center" style={{ filter: `drop-shadow(0 0 6px ${glow})` }}>{icon}</div>
  ...
  return (
    <section className="relative space-y-8">
      ...
      <motion.div
        ...
        style={{ background: 'rgba(0,0,0,0.6)', border: '1px solid rgba(241,196,15,0.18)', boxShadow: '0 0 60px rgba(241,196,15,0.05), 0 0 120px rgba(192,57,43,0.04)' }}>
          ...
          <div className="relative shrink-0">
            <motion.div
              ...
              <motion.div
                ...
                <span className="text-[9px] font-semibold uppercase tracking-wider">{uploading ? 'Upload...' : 'Ganti'}</span>
              </motion.div>
            </motion.div>
      ...
      <div>
        ...
        {played > 0 && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.45 }} className="mt-4 space-y-1.5">
            <div className="flex items-center justify-between">
              ...
              <motion.span className="text-xs font-bold" style={{ color: '#4ade80' }}
                initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.7 }}>{winRate}%</motion.span>
            </div>
      ...
      <div>
        ...
        <div className="grid gap-3 sm:grid-cols-2">
          {[
            { title:'Lawan Bot', desc:'Bermain melawan AI. Cocok untuk latihan dan menguasai strategi Kartu Batak.', icon:<IconBot/>, action:()=>navigate('/game'), btnLabel:'Main Sekarang', gradient:'linear-gradient(135deg,#8b1a12,#c0392b,#e74c3c)', glow:'rgba(192,57,43,0.55)', glowBright:'rgba(192,57,43,0.8)', border:'rgba(192,57,43,0.28)', bgAccent:'rgba(192,57,43,0.1)', delay:0.4 },
            { title:'Multiplayer Online', desc:'Tantang pemain lain secara real-time. Buat room atau join room teman.', icon:<IconGlobe/>, action:()=>navigate('/lobby'), btnLabel:'Masuk Lobby', gradient:'linear-gradient(135deg,#1a0a4a,#3d1a8a,#5b1fa0)', glow:'rgba(91,31,160,0.55)', glowBright:'rgba(142,68,173,0.8)', border:'rgba(142,68,173,0.28)', bgAccent:'rgba(142,68,173,0.08)', delay:0.48 },
          ].map((m) => (
            <motion.div key={m.title}
              ...
              style={{ border: `1px solid ${m.border}`, background: 'rgba(0,0,0,0.55)', transition: 'box-shadow 0.3s ease' }}>
              <div className="absolute inset-x-0 top-0 h-px"
              ...
              <div className="relative">
                <div className="mb-4 flex items-center gap-3">
                  <motion.div className="flex h-11 w-11 items-center justify-center rounded-xl"
                    style={{ background: m.bgAccent, border: `1px solid ${m.border}`, color: '#f5d87a' }}
                    whileHover={{ rotate: [0, -8, 8, 0] }} transition={{ duration: 0.4 }}>
                    {m.icon}
                  </motion.div>
                  <p className="font-perpetua text-xl" style={{ color: '#f5d87a', textShadow: '0 0 10px rgba(241,196,15,0.2)' }}>{m.title}</p>
                </div>
                ...
                <motion.button type="button" onClick={m.action}
                  whileHover={{ scale: 1.04, boxShadow: `0 0 24px ${m.glow}` }} whileTap={{ scale: 0.97 }}
      ...
      {user && (
        <div>
          <motion.div
            ...
            className="mb-4 flex items-center gap-3">
            <div className="h-px w-5" style={{ background: 'rgba(241,196,15,0.4)' }} />
            <p className="text-[10px] uppercase tracking-[0.45em]" style={{ color: 'rgba(241,196,15,0.45)' }}>Bonus & Misi Harian</p>
            <div className="h-px flex-1" style={{ background: 'linear-gradient(90deg,rgba(241,196,15,0.2),transparent)' }} />
          </motion.div>
          <DailyPanel userId={user.id} />
        </div>
      )}
